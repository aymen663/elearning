...
router.get('/me/certificates', protect, async (req, res) => {
    try {
        // Aggregation avoids transmitting lesson content; only counts lessons
        const certificates = await Progress.aggregate([
            { $match: { student: req.user._id } },
            {
                $lookup: {
                    from: 'courses',
                    ...
                    pipeline: [
                        {
                            $project: {
                                title: 1, thumbnail: 1, category: 1, level: 1,
            ...
            { $unwind: '$courseData' },
            {
                $addFields: {
                    completionPercentage: {
                        $cond: [
                            { $gt: ['$courseData.lessonCount', 0] },
                            { $round: [{ $multiply: [{ $divide: [{ $size: '$completedLessons' }, '$courseData.lessonCount'] }, 100] }, 0] },
                            0,
                        ],
                    },
                },
            },
            { $match: { completionPercentage: 100 } },
            {
                $project: {
                    course: '$courseData',
                    completionPercentage: 1,
                    completedAt: '$lastAccessed',
                },
            },
        ]);
