            ...
            <motion.section initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}
                ...
                <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-2">
                    <motion.p initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}
                        transition={{ delay: 0.1 }} className="font-black text-sm" style={{ color: G }}>
            ...
            <motion.div initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                ...
                <div className="flex items-center gap-2.5">
                    <motion.div whileHover={{ rotate: 15, scale: 1.1 }} transition={{ type: 'spring', stiffness: 300 }}
                        ...
                        <GraduationCap className="w-3.5 h-3.5" style={{ color: G }} />
                    </motion.div>
                    <span className="text-white/25 text-sm">© {new Date().getFullYear()} EduAI</span>
                </div>
                ...
                <div className="flex items-center gap-5">
                    {LINKS.map(({ label, href }, i) => (
                        <motion.div key={label} initial={{ opacity: 0, y: 8 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                            ...
                            <Link href={href} className="text-white/30 text-xs hover:text-white transition-colors">{label}</Link>
                        </motion.div>
                    ))}
                </div>
                ...
                <div className="flex items-center gap-2">
                    {SOCIALS.map(({ icon: Icon, href, label }, i) => (
                        <motion.a key={label} href={href} target="_blank" rel="noopener noreferrer" title={label}
                            ...
                            className="w-7 h-7 rounded-full border border-white/[0.08] flex items-center justify-center text-white/25 hover:text-white hover:border-white/20 transition-colors">
